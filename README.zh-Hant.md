...
## 裝備評分（F7）
按 **F7** 開啟**裝備評分面板** —— 仿 WoW 裝等的「戰力一眼看」評分。每件裝備依 **稀有度 + 裝備等級 + 詞綴 + 已鑲嵌數**
計分,逐角色加總成 **總分**,並拆成 **⚔ 攻擊 / ⛨ 防禦** 兩個數字。每一列顯示裝備 **icon**、依**稀有度上色**的名稱
(TBH 十階:普通 → … → 不朽 → 宇宙)、裝備等級、鑲嵌點 ◆ 與該件分數。按 **詳細** 可展開每件的逐效果分數明細。

詞綴與鑲嵌數讀自你的存檔;稀有度、裝備等級、icon 來自內建的 wiki 對照表(以物品為 key)—— 所以**離線可用**、改版也不怕。
詞綴名稱跟著遊戲語言走。
...
**滑鼠中鍵**點物品可以把價格框**釘選**在它身上 —— 釘選後框會固定，你可以把滑鼠移到曲線上，
...
- **滑鼠中鍵點物品** — 釘選／取消釘選價格框（釘選後滑過曲線可讀每一天）
