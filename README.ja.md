...
**TaskBarHero**（TBH: Task Bar Hero）向けのゲーム内 **DPS / 被ダメージ / ステージ比較 / 装備スコア / アイテム集計 / 周回効率プランナー / 宝箱ログ / 開封統計 / ドロップ分布 / Steam マーケット価格チェック** オーバーレイ。すべて **F1 コントロール（コントロールセンター）** から表示切替できます。
...
## アイテム集計（F8）
**F8** で**アイテム集計パネル**を開きます —— **バックパック＋倉庫（保管庫）＋取引保管庫**に持っているすべてを集計します
（装備中のギアは除外）。スクロール可能な単一リストに、1 アイテム 1 行（**アイコン** ＋**レア度で色分け**した名前 ＋所持 **×N**）で表示し、
同一アイテムはまとめます。リスト上部の 2 段のフィルターチップで**レア度**と**種類**で絞り込め —— 組み合わせ可能（例：イモータル × 指輪）、
もう一度クリックで解除でき、各チップの個数はもう一方の有効なフィルターと**連動**して更新されます。

レア度・種類・名前・アイコンは内蔵の wiki テーブル（アイテム key）から解決するため、**オフライン**で動作し 5 言語の UI すべてに対応 ——
非装備（素材・ボックス）もカバーします。サマリー行は バッグ／倉庫／取引 を **使用数／容量**で表示し、倉庫容量は有効なタブを反映します
（4 タブを開放しきった倉庫は 196/343 ではなく 196/196 と表示）。

<img src="image/倉庫頁面.png" alt="アイテム集計パネル" width="300">

> *スクロール可能な単一リスト、1 アイテム 1 行でアイコン・レア度で色分けした名前・所持数を表示。上部にレア度と種類のフィルターチップ、個数は連動。サマリーは バッグ／倉庫／取引 の使用数 vs 容量。*
...
プレイヤーごとのスクレイピングは不要です。アイテムを**中クリック**するとボックスをそのアイテムに**ピン留め**でき ——
...
- **F8** — アイテム集計パネルの表示/非表示（設定可：`ItemStatsUI.ToggleKey`）
...
- **アイテムを中クリック** — 価格ボックスをピン留め／解除（ピン後はカーブにホバーして各日を確認）
