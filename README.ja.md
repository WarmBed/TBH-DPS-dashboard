...
## 装備スコア（F7）
**F7** で**装備スコアパネル**を開きます —— WoW の装備レベル風に、パーティ全体の戦力をひと目で。各装備を
**レア度 + 装備レベル + affix + 装着済みソケット数**でスコア化し、キャラごとに**合計**へ集計、さらに
**⚔ 攻撃 / ⛨ 防御**に分割します。各行に装備 **アイコン**、**レア度で色分け**した名前（TBH の10段階：
コモン → … → イモータル → コズミック）、装備レベル、ソケット ◆、スコアを表示。**詳細**でアイテムごとの
効果別スコア内訳を展開できます。

affix とソケット数はセーブから読み取り、レア度・装備レベル・アイコンは内蔵の wiki テーブル（アイテム key）
から解決 —— **オフライン**で動作し、ゲーム更新にも強い。affix 名はゲーム言語に追従します。

<img src="image/裝備評分.png" alt="装備スコアパネル" width="300">

> *キャラごとの合計＋攻撃/防御の分割。各装備はレア度で色分けし、アイコン・レベル・ソケット・スコアを表示。詳細モードで各装備を採点項目に分解。*
...
- **F7** — 装備スコアパネルの表示/非表示（設定可：`GearScoreUI.ToggleKey`）
